...
Generator.prototype.values = function (valuesList) {
  ...
  if (valuesList.from) {
    return 'VALUES ' + lparen + keys.join(' ') + rparen + ' FROM ' + this.toEntity(valuesList.from);
  }
...
Generator.prototype.namedQuery = function (namedQuery) {
  return 'QUERY ' + this.toEntity(namedQuery.name) + ' ' + this.group(namedQuery);
};
